...
const Students = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const apiUrl =
          "https://vsbec-placement-backend.onrender.com/student/all";
  ...
  const getBorderColor = (item) => {
    if (todayAttendance) {
      return item.attendance.some(
        (entry) => entry.date === todayDateString && entry.isPresent
      )
        ? "green"
        : "gray";
    } else {
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <TouchableOpacity
          ...
          style={styles.todayAttendanceButton}
        >
          <Text style={styles.todayAttendanceButtonText}>
            Today's Attendance
          </Text>
        </TouchableOpacity>
      </View>
                ...
                <FontAwesome
                  name={
                    item.attendance.some(
                      (entry) =>
                        entry.date === todayDateString && entry.isPresent
                    )
                      ? "check-circle"
                      : "times-circle-o"
                  }
